...
import { Trophy, MapPin, Navigation, Fuel, ChevronDown, ChevronUp } from "lucide-react";
...
const MEDAL_COLORS = [
  "bg-[hsl(45,93%,47%)] text-white", // gold
  "bg-[hsl(0,0%,75%)] text-white",   // silver
  "bg-[hsl(29,60%,45%)] text-white",  // bronze
];
...
const NearbyRanking = ({ stations, userLocation, onNavigate }: NearbyRankingProps) => {
  ...
  const ranking = useMemo(() => {
    ...
    return stations
      ...
      .filter((s) => (s.prices[activeFuel] ?? 0) > 0)
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div className="text-left">
            ...
            <p className="text-[10px] text-muted-foreground">Estaciones en 15 km · {fuelInfo.label}</p>
          </div>
        </div>
        {expanded ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-[9px] text-muted-foreground">CLP/L</p>
                  </div>
